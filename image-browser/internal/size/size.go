...
func Load(filename string) ([]Group, error) {
	...
	for _, record := range records {
		if record.Status != manifest.StatusProcessed {
			continue
		}
		...
		// Cache existence per unique file so a file referenced by
		// multiple records is only stat'd once.
		ok, checked := exists[record.File]
		if !checked {
			_, err := os.Stat(record.File)
			ok = err == nil
			exists[record.File] = ok
		}
		if !ok {
			continue
		}
