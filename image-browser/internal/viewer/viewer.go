...
//
// destination is held in a mutable field, rather than captured directly by
// reviewMoveAll's closure, so changeMoveAllDestination can redirect Move All
// to a different folder without restarting the command.
func (b *Browser) SetMoveAll(destination string, done func()) {
	b.moveAllDest = destination
	b.moveAllDone = done
	b.reviewMoveAll = func() {
		b.confirmMoveAll(b.moveAllDest, b.moveAllDone)
	}
	b.single = b.buildSingleView()
}
...
func (b *Browser) reviewToolbar() fyne.CanvasObject {
	...
	if b.reviewMoveAll != nil {
		...
		buttons = append(buttons, widget.NewButton("Change Destination", func() {
			b.changeMoveAllDestination()
		}))
	}
	if b.reviewQuit != nil {
...
// activeImages returns the list that single-image navigation (openImage,
// next/previous, trash, copy path, preloading) should operate over. When a
// directory preview is open, navigation is scoped to that directory's images
// rather than the full root-level list.
func (b *Browser) activeImages() []string {
	if b.previewDir != "" {
		return b.previewImages
	}
	return b.images
}

func (b *Browser) openImage(path string) {
	for i, p := range b.activeImages() {
		if p == path {
			b.index = i
			break
		}
...
func (b *Browser) displayFullImage(path string, img image.Image) {
	active := b.activeImages()
	if len(active) == 0 || active[b.index] != path {
		return
	}
...
func (b *Browser) loadFullImageAsync(path string, generation uint64, display bool) {
	...
	go func() {
		...
		if img == nil {
			...
			fyne.Do(func() {
				active := b.activeImages()
				if generation != b.currentLoadGeneration() || len(active) == 0 || active[b.index] != path {
					return
				}
				b.image.Image = nil
				b.image.Refresh()
...
func (b *Browser) preloadNeighbors() {
	...
	if b.index+1 < len(active) {
		b.loadFullImageAsync(active[b.index+1], generation, false)
	}
	if b.index > 0 {
		b.loadFullImageAsync(active[b.index-1], generation, false)
	}
...
func (b *Browser) nextImage() {
	if !b.inImageView() || b.index >= len(b.activeImages())-1 {
		return
	}
	b.index++
	b.showImage()
...
// removeImages drops paths from b.images and, if a directory preview is
// open, from b.previewImages too, along with their cached decodes. b.index
// is clamped so it still points at a valid entry in whichever list
// navigation is currently scoped to (or stays at 0 if empty).
func (b *Browser) removeImages(paths []string) {
	...
	b.images = filterOutPaths(b.images, remove)
	if b.previewImages != nil {
		b.previewImages = filterOutPaths(b.previewImages, remove)
	}

	if active := len(b.activeImages()); b.index >= active {
		b.index = max(active-1, 0)
	}
}

// filterOutPaths returns paths with every entry present in remove dropped,
// reusing paths' backing array.
func filterOutPaths(paths []string, remove map[string]struct{}) []string {
	kept := paths[:0]
	for _, path := range paths {
...
func (b *Browser) trashCurrentImage() {
	if !b.trashEnabled || !b.inImageView() {
		return
	}
	active := b.activeImages()
	if len(active) == 0 {
...
func (b *Browser) copyCurrentPath() {
	if active := b.activeImages(); len(active) > 0 {
		b.copyText(active[b.index])
	}
}
