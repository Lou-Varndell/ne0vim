...
func totalImageReferences(data []manifest.Entry) int {
	...
	for _, item := range data {
		if item.Status != "missing" && item.Status != "non-image" && item.Status != "invalid" && item.File != "" {
			count++
		}
	}
	return count
...
func processImages(ctx context.Context, inputFile, outputFile string, maxWorkers, checkpointEvery int) error {
	...
	for i, item := range data {
		if item.Status == "missing" || item.Status == "non-image" || item.Status == "invalid" {
			continue
		}
